...
(function () {
	...
	Basebullet = function (config) {
		config = config || {};
		return this.__construct(config);
	};

	Basebullet.prototype = lib.extend(BaseCharacter.prototype, {
		...
		_bulletExplosion: function (config) {
			...
			_.each(self._targets, function (target, index) {
				...
				if (self._hitRect({
					...
					//ダメージ状態に
					target._instance._isDamage = true;
					//HPを減らす
					target._instance._hitpoint = target._instance._hitpoint - 1;
					if(target._instance._hitpoint <= 0){
						target._instance._hitRotation({
							complete: function () {
						...
						//キャラが跳ね返る動きの実行
						target._instance._hitReaction({
							target: target._instance.chara,
							complete: function () {
								//ダメージ状態から通常状態に戻す
								target._instance._isDamage = false;
								return;
							}
						});
					}
